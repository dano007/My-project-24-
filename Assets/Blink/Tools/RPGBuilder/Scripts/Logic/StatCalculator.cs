...
namespace BLINK.RPGBuilder.Logic
{
    public static class StatCalculator
    {
        ...
        private static void TriggerNPCMoveSpeedChange(CombatNode cbtNode)
        {
            cbtNode.agentREF.SetSpeed(RPGBuilderUtilities.getCurrentMoveSpeed(cbtNode));
        }
        ...
        public static void HandleStat(CombatNode cbtNode, RPGStat statREF, CombatNode.NODE_STATS nodeStatData, float amount, bool isPercent, TemporaryStatSourceType sourceType)
        {
            ...
            if(cbtNode.nodeType == CombatNode.COMBAT_NODE_TYPE.mob || cbtNode.nodeType == CombatNode.COMBAT_NODE_TYPE.pet) TriggerNPCMoveSpeedChange(cbtNode);
        }
        ...
        private static void ResetEffectsStats(CombatNode cbtNode)
        {
            ...
            switch (cbtNode.nodeType)
            {
                ...
                case CombatNode.COMBAT_NODE_TYPE.pet:
                    if (moveSpeedChanged) TriggerNPCMoveSpeedChange(cbtNode);
                    break;
        ...
        private static void ProcessTempStatList(TemporaryStatSourceType sourceType, CombatNode cbtNode)
        {
            foreach (var t in tempStatList)
            {
                foreach (var t2 in cbtNode.nodeStats)
                {
                    ...
                    if (RPGBuilderUtilities.isStatAffectingMoveSpeed(t2.stat))
                    {
                        switch (cbtNode.nodeType)
                        {
                            case CombatNode.COMBAT_NODE_TYPE.mob:
                            case CombatNode.COMBAT_NODE_TYPE.pet:
                                TriggerNPCMoveSpeedChange(cbtNode);
                                break;
                            case CombatNode.COMBAT_NODE_TYPE.player:
                                TriggerMoveSpeedChange();
                                break;
