...
namespace BLINK.RPGBuilder.World
{
    public class InteractiveNode : MonoBehaviour, IPlayerInteractable
    {
        ...
        public void UseNode()
        {
            ...
            if ((InteractiveNode) WorldInteractableDisplayManager.Instance.cachedInteractable == this)
            ...
            foreach (var t in useRequirement.Where(t =>
                t.requirementType == RequirementsManager.RequirementType.itemOwned && t.consumeItem))
            {
                InventoryManager.Instance.RemoveItem(t.itemRequiredID, t.itemRequiredCount, 0, -1, false);
            }
        }
        ...
        private void OnMouseOver()
        {
            ...
            {
                float maxDistance = useDistanceMax;
                if (nodeType == InteractiveNodeType.resourceNode)
                {
                    var curRank = RPGBuilderUtilities.getResourceNodeRank(resourceNodeData.ID);
                    var rankREF = resourceNodeData.ranks[curRank];
                    useDistanceMax = rankREF.distanceMax;
                }

                if (Vector3.Distance(transform.position, CombatManager.playerCombatNode.transform.position) <= maxDistance)
                {
