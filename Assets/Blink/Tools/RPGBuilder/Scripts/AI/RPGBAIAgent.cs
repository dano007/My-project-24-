...
namespace BLINK.RPGBuilder.AI
{
    public class RPGBAIAgent : MonoBehaviour
    {
        ...
        public void ResetStun()
        {
            ...
            moveSpeed = CombatManager.Instance.GetTotalOfStatType(thisCombatInfo, RPGStat.STAT_TYPE.MOVEMENT_SPEED);
            thisAgent.speed = moveSpeed;
            thisAgent.acceleration = moveSpeed * 3;
        }
        ...
        {
            float dist = Vector3.Distance(cached_SpawnPos, target.transform.position);
        ...
        public bool IsAttackerTooFar(CombatNode attacker)
        {
            float dist = Vector3.Distance(cached_SpawnPos, attacker.transform.position);
            return dist > (GetRoamRange() + GetResetDistance());
        }
